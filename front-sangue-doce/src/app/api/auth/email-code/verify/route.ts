...
import { api, type VerifyEmailLoginCodePayload } from "@/lib/api";
...
export async function POST(request: Request) {
  const payload = (await request.json()) as VerifyEmailLoginCodePayload;
  const loginResponse = await api.auth.verifyEmailCode(payload).catch((error: unknown) => {
  ...
  if (!loginResponse?.access_token) {
    return NextResponse.json({ message: "Codigo invalido ou expirado." }, { status: 401 });
  }
