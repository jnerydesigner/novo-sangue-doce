...
export const MEASUREMENT_NOTE_SCHEDULE: Partial<
  Record<MeasurementNoteType, { hour: number; minute: number }>
> = {
  FASTING_WAKE_UP: { hour: 5, minute: 30 },
  BEFORE_BREAKFAST: { hour: 6, minute: 0 },
  AFTER_BREAKFAST: { hour: 8, minute: 0 },
  BEFORE_LUNCH: { hour: 11, minute: 0 },
  AFTER_LUNCH: { hour: 14, minute: 0 },
  BEFORE_DINNER: { hour: 18, minute: 0 },
  AFTER_DINNER: { hour: 22, minute: 0 },
  BEFORE_SLEEP: { hour: 23, minute: 0 },
};
...
export function classifyMeasurementMoment(
  ...
  const rule = MEASUREMENT_MOMENT_RULES.find(
    (item) => measuredMinute >= item.startMinute && measuredMinute <= item.endMinute,
  );
