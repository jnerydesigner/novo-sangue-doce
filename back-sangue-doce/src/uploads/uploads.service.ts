...
export class UploadsService {
  ...
  constructor(
    ...
    this.publicPrefix =
      configService.get<string>('MINIO_PUBLIC_PREFIX') ?? DEFAULT_PUBLIC_PREFIX;
  ...
  async uploadUserAvatar(
    ...
    await this.userRepository.updateAvatarUrl(publicUser.id, avatarUrl);
  ...
  private validateImage(
    ...
    if (!file) {
      throw new BadRequestException(
        'Envie a imagem no campo "image" usando multipart/form-data.',
      );
    }
  ...
  private async createBucketIfMissing(): Promise<void> {
    try {
      ...
      throw new InternalServerErrorException(
        'Nao foi possivel preparar o bucket de uploads.',
        { cause: error },
      );
    }
  }
  ...
  private getClient(): Client {
    ...
    const port = Number(
      this.configService.get<string>('MINIO_API_PORT') ?? 9000,
    );
    ...
    if (!accessKey || !secretKey) {
      throw new InternalServerErrorException(
        'As credenciais do MinIO nao estao configuradas.',
      );
    }
